...
const useFirmasExistentes = () => {
    ...
    const getPresignedUrl = useCallback(async (s3Key: string): Promise<string | null> => {
        try {
    ...
    const cargarFirmas = useCallback(async (liquidacionId: string) => {
        try {
            setLoading(true);
            const { data } = await apiClient.get(`/api/firmas_desprendible/liquidacion/${liquidacionId}`);

            if (data?.success && data?.data) {
                const firmasActivas = data.data.filter((firma: Firma) => firma.estado === "Activa");
                
                // Inicializar con loading state
                const firmasInicial: FirmaConUrl[] = firmasActivas.map((firma: Firma) => ({
                    ...firma,
                    urlLoading: true,
                    urlError: false
                }));
                ...
                const promesasUrls = firmasActivas.map(async (firma: Firma, index: number) => {
                    try {
...
export default function Page() {
    ...
    // Estados principales
    const [liquidacionData, setLiquidacionData] = useState<Liquidacion | null>(null);
    ...
    const { firmas, loading: firmasLoading, cargarFirmas } = useFirmasExistentes();
    ...
    if (error && !liquidacionData) {
        return <ErrorState error={error} onRetry={() => fetchLiquidacion(liquidacionId)} />;
    }
...
const PageHeader = ({ documentoFirmado }: { documentoFirmado: boolean }) => (
    <div className="mb-6 text-center">
        ...
        <p className="text-gray-600">
            {!documentoFirmado && "Complete el proceso de firma para acceder a su desprendible de pago"}
        </p>
    </div>
);

const DocumentStatus = ({ documentoFirmado }: { documentoFirmado: boolean }) => (
    <div className="flex items-center gap-2 w-full justify-center">
        {documentoFirmado ? (
            <>
                <DocumentCheckIcon className="h-5 w-5 text-green-600" />
                <h2 className="text-xl font-semibold text-green-600">Documento Firmado</h2>
            </>
        ) : (
            <>
                <PencilIcon className="h-5 w-5 text-emerald-600" />
                <h2 className="text-xl font-semibold text-emerald-600">Tablero de Firma Digital</h2>
            </>
        )}
    </div>
);
...
const ConductorInfo = ({ 
    ...
    <div className={`p-4 rounded-lg ${documentoFirmado ? "bg-green-50" : "bg-emerald-50"}`}>
        <h3 className={`font-medium mb-2 ${documentoFirmado ? "text-green-600" : "text-emerald-600"}`}>
            Información del Conductor
        </h3>
        <div className="space-y-1">
            <InfoItem label="Nombre" value={info.nombreCompleto} documentoFirmado={documentoFirmado} />
            <InfoItem label="Período" value={info.periodo} documentoFirmado={documentoFirmado} />
            <InfoItem label="ID" value={info.id} documentoFirmado={documentoFirmado} />
        </div>
    </div>
);
...
const InfoItem = ({ 
    ...
    <p>
        <span className={`font-medium ${documentoFirmado ? "text-green-600" : "text-emerald-600"}`}>
            {label}:
        </span>{" "}
        {value}
    </p>
            ...
            <div className="flex items-start">
                ...
                <div>
                    ...
                    <p className="text-sm text-green-700 mt-1">
                        Este desprendible de nómina ya ha sido firmado digitalmente. 
                        Puede proceder a visualizar o descargar el documento.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div className="flex items-start">
            ...
            <div>
                ...
                <p className="text-sm text-yellow-700 mt-1">
                    Para poder visualizar o descargar su desprendible de nómina, 
                    debe firmar digitalmente confirmando que ha recibido el documento.
                </p>
            </div>
        </div>
    </div>
        ...
        <ModalContent>
            {(onClose) => (
                <>
                    ...
                    <ModalBody>
                        <p className="mb-4">
                            Su firma ha sido registrada y guardada de forma segura. 
                            El documento ya está disponible para su consulta.
                        </p>
