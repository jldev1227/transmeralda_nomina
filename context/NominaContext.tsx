...
export interface Liquidacion {
    ...
    periodoStartVacaciones?: string | null;
    periodoEndVacaciones?: string | null;
...
export const NominaProvider: React.FC<NominaProviderProps> = ({ children }) => {
    ...
    const filtrarLiquidaciones = useCallback((): Liquidacion[] => {
        ...
        let filteredLiquidaciones = liquidaciones.filter(liquidacion => {
          ...
          // Filtro por fecha de inicio del periodo
          if (filtros.periodoStart && liquidacion.periodoStart) {
            const fechaFiltro = new Date(filtros.periodoStart);
            const fechaLiquidacion = new Date(liquidacion.periodoStart);
            if (fechaLiquidacion < fechaFiltro) {
              return false;
            }
          }
          
          // Filtro por fecha de fin del periodo
          if (filtros.periodoEnd && liquidacion.periodoEnd) {
            const fechaFiltro = new Date(filtros.periodoEnd);
            const fechaLiquidacion = new Date(liquidacion.periodoEnd);
            if (fechaLiquidacion > fechaFiltro) {
              return false;
            }
          }
          ...
          if (filtros.busqueda && filtros.busqueda.trim() !== '') {
            ...
            const periodoStart = liquidacion.periodoStart ? liquidacion.periodoStart.toLowerCase() : '';
            const periodoEnd = liquidacion.periodoEnd ? liquidacion.periodoEnd.toLowerCase() : '';
    ...
    const calcularEstadisticas = useCallback((): EstadisticasLiquidacion => {
        ...
        return {
            ...
            montoTotal: liquidacionesFiltradas.reduce((sum, liq) => sum + (parseFloat(liq.sueldoTotal.toString()) || 0), 0)
        };
